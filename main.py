...
# --- Function ---
    ...
    # initialize SESSIONS @ sid with all ids & ts (json)
...
def end_session(body: StartSessionOut):
...
def get_timeline(session_id: str, limit: int = Query(50, ge=1, le=200)):
    if session_id not in (SESSIONS or EVENTS_BY_SESSION):
        raise HTTPException(status_code=404, detail = "Session not found")
